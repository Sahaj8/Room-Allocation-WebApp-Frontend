...
const ActivityList = () => {
    ...
    useEffect(() => {
        const getActivityList = async () => {
            const response = await axios.get("https://e022-103-156-19-229.in.ngrok.io/activity");
            const activityData = await response.data;
            setActivityList(activityData);
            const token = localStorage.getItem('token');
            if(token) {
                const userDataresponse = await axios.get("https://e022-103-156-19-229.in.ngrok.io/users/", {headers: { Authorization: token },})
